class CorporateNavigation extends HTMLElement {
    constructor() {
        super();
        this.navItems = [
            { id: 'advantages', icon: 'star', text: 'Переваги' },
            { id: 'fleet', icon: 'car-side', text: 'Корпоративний автопарк' },
            { id: 'leasing', icon: 'file-contract', text: 'Лізинг' },
            { id: 'service', icon: 'tools', text: 'Сервісне обслуговування' },
            { id: 'contact', icon: 'envelope', text: 'Зв\'язатися з нами' }
        ];
    }
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700">
                    ...
                    <div class="md:hidden">
                        <div class="grid grid-cols-3 gap-1 p-2">
                            ${this.navItems.map(item => `
                                <a href="#${item.id}" class="flex flex-col items-center justify-center p-3 rounded-lg text-center transition-colors duration-200 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600 dark:hover:text-indigo-400">
...
customElements.define('corporate-navigation', CorporateNavigation); 